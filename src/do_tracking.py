...
def main():
    ...
    for i in range(1, len(site0)):
        cur_frame_nuc = read_nd2.get_channel_rawData(movie, 0, i, 0)
        curr_rbg = np.dstack((cur_frame_nuc, cur_frame_nuc, cur_frame_nuc))
        curr_rbg = cv2.normalize(curr_rbg, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
        master_cells = tracking_utils.link_next_frame(master_cells, curr_rbg, i)
